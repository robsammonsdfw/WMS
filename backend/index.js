...
async function initSchema(client) {
    ...
    await client.query(`
        ...
        -- 6. Accounts and other existing tables...
...
exports.handler = async (event) => {
    ...
    try {
        if (path === '/admin/reset-db' && httpMethod === 'POST') {
            ...
            // Seed Laterals
            await client.query("INSERT INTO laterals (id, name) VALUES ('L1', 'Lateral A'), ('L2', 'Lateral B')");
            // Seed Headgates
            await client.query("INSERT INTO headgates (id, name, lateral_id, tap_number) VALUES ('HG1', 'Gate A-1', 'L1', 'A-12'), ('HG2', 'Gate B-1', 'L2', 'B-05')");
            // Seed Fields
            await client.query("INSERT INTO fields (id, name, crop, acres, total_water_allocation, water_used) VALUES ('F1', 'North Field', 'Alfalfa', 100, 400, 50)");
            await client.query("INSERT INTO field_headgates (field_id, headgate_id) VALUES ('F1', 'HG1')");
            return { statusCode: 200, headers, body: JSON.stringify({ message: "Seeded" }) };
        }

        if (path === '/laterals' && httpMethod === 'GET') {
            const res = await client.query("SELECT * FROM laterals");
        ...
        if (path === '/headgates' && httpMethod === 'GET') {
            const res = await client.query("SELECT * FROM headgates");
            return { statusCode: 200, headers, body: JSON.stringify(res.rows) };
        }
        if (path === '/headgates' && httpMethod === 'POST') {
            const data = JSON.parse(body);
            if (!data.lateralId) throw new Error("Headgates must be assigned to a lateral.");
        ...
        if (path === '/orders' && httpMethod === 'GET') {
            const res = await client.query("SELECT * FROM water_orders ORDER BY delivery_start_date ASC");
            return { statusCode: 200, headers, body: JSON.stringify(res.rows) };
        }
